...
const MyProducts = (props) => {
  ...
  <Box
    component="span"
    sx={{ display: 'inline-block', mx: '2px', transform: 'scale(0.8)'  ,'& > :not(style)': {
          m: 2,
        }, }}
  >
    •
 ...
 const AddToFavoris=(item)=>{
     createFavoriteList(
        {
            ...
            variables:{
                input:{ProductId:item._id,productName:item.name,productImg:item.productImg,user:userConecte}
            }}
    )
    if(error){
        console.log(error)
  ...
  return (
    ...
    {data?.products?.map((item)=>{
       const ExistingFavorite = dataFavoris?.listFavorisByUser?.some((elem)=>{
      return elem.ProductId===  item?._id;
